...
export default function MediaSection() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const password = "admin"; // auth from backend as needed
      ...
      try {
        ...
        let items = photoRes.data.messageWithImg.map((item) => ({
          id: item.id,
          header: item.header,
          order: item.order,
        ...
        // If less than 3 photos, fallback to fake API
        if (items.length < 3) {
          const res = await fetch("https://fakestoreapi.com/users");
          const data = await res.json();

          items = data.slice(0, 6).map((user, i) => ({
            id: user.id,
